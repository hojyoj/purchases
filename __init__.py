...
class Controller(processes.Controller):
	...
	def init ( self ) :
		...
		self.app.log2sys ( 'info', "			Controller.init()								@ purchases" )
		...
		try :
			...
			self.app.log2sys ( 'error', "	Tipos de documento no disponibles" )
			self.app.showMessage( 'critical', 'Error : 85456' )

			raise

		self.activePriceRules = self.app.model.get_activePriceRules()

		self._purchase_documentKind_default = self.defaultDocumentKind(cast_='purchase')

		self.app.log2sys ( 'info', "			Controller.init() - end							@ purchases\n" )
	...
	def cfdis_save(self, xmlFile):
		...
		"""
		self.app.log2sys ( 'info', "    purchases Controller.cfdis_save()" )
		...
		self.app.log2sys ( 'info', "    purchases Controller.cfdis_save() - END" )
	...
	def newCfdis_pull(self):
		...
		self.app.log2sys ( 'info', "    purchases   Controller.newCfdis_pull()" )
		...
		for document in documents:
			...
			if not existent:
				...
				self.app.log2sys ( 'info', ">>>> {} Must process {}".format(documents.index(document), document.data) )
			else :
				self.app.log2sys ( 'info', ">>>> {}".format(documents.index(document)) )

		self.app.log2sys ( 'info', "    purchases   Controller.newCfdis_pull() - END" )
	...
	def orders_load(self, **kwds):
		self.app.log2sys ( 'info', "    purchases Controller.orders_load()" )
		...
		self.app.log2sys ( 'info', "    purchases Controller.orders_load() - END" )
	...
	def process_save(self, **data):
		# oldLevel = self.app.stdoutLog.level
		# self.app.stdoutLog.setLevel(logging.INFO)
		self.app.log2sys ( 'info', "    purchases Controller.process_save()" )
		...
		self.app.log2sys ( 'info', "    purchases Controller.process_save() - END" )
		# self.app.stdoutLog.setLevel(oldLevel)
